...
const Page = async () => {
    ...
    const [userInterviews, latestInterviews] = await Promise.all([
        await getInterviewByUserId(loggedInUserId), // Use the logged-in user's ID
        await getLatestInterviews({ userId: loggedInUserId }) // Use the logged-in user's ID
    ]);
                    ...
                    {hasPastInterviews ? (
                        userInterviews?.map((interview) => (
                            <InterviewCard
                                {...interview}
                                key={interview.id}
                                userId={loggedInUserId} // Ensure the logged-in user's ID is passed
                            />
                        ))
                    ) : (
